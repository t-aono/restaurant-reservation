          ...
          <v-card color="grey lighten-4" min-width="350px" flat>
            <v-toolbar :color="selectedEvent.color" dark>
              ...
              <v-btn
                text
                color="grey"
                @click="editReservation(selectedEvent.index)"
                v-if="selectedEvent.status === '仮予約' && type === 'day'"
              >
              ...
              <v-btn
                text
                color="green"
                @click="fixReservation(selectedEvent.index)"
                v-if="selectedEvent.status === '仮予約' && type === 'day'"
              >
              ...
              <v-btn
                text
                color="lime"
                @click="resetFix(selectedEvent.index)"
                v-else
              >
              ...
              <v-btn
                ...
                @click="cancelReservation(selectedEvent.index)"
                v-if="type === 'day'"
              >
...
export default {
  ...
  methods: {
    ...
    editReservation(index) {
      ...
      this.selectedOpen = false;
      this.newReserve = true;
      this.name = this.events[index].name.replace(' 様', '');
      this.mail = this.events[index].mail;
      this.time = this.events[index].start.getHours() + ":" + ("0" + this.events[index].start.getMinutes()).slice(-2);
      this.people = this.events[index].people;
      this.remarks = this.events[index].remarks;
    },
    fixReservation(index) {
      // 予約の確定
      this.events[index].status = "予約";
      this.events[index].color = "green";
      db.collection("reservations").doc(this.ym).set({
        events: this.events,
      })
      .then(() => {
        this.selectedOpen = false;
    ...
    resetFix(index) {
      // 未確定に
      this.events[index].status = "仮予約";
      this.events[index].color = "lime";
      db.collection("reservations").doc(this.ym).set({
        events: this.events,
      })
      .then(() => {
    ...
    cancelReservation(index) {
      ...
      db.collection("reservations").doc(this.ym)
      .set({
        events: this.events,
      })
      .then(() => {
